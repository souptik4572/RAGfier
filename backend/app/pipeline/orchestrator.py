...
class IngestionOrchestrator:
    ...
    def __init__(
        ...
        # Anthropic-style situated context (Contextual Retrieval). Disabled
        # by default; enabled via settings.contextualization_enabled +
        # ANTHROPIC_API_KEY. When disabled this is a no-op.
        self._contextualizer = contextualizer or Contextualizer()
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            # --- Contextualize (Anthropic-style situated context) ---
            # Runs only when the feature flag + API key are set. The
            # contextualizer prepends a "[Context: …]" summary generated by
            # Claude Haiku, using prompt caching on the document body to
            # keep per-chunk cost ~10% of the naive approach. Failures are
            # swallowed inside the contextualizer so ingestion is resilient.
            if self._contextualizer.enabled:
                await asyncio.to_thread(
                ...
                enriched_contents = await self._contextualizer.contextualize_chunks(
                    document_text=document_text,
                    chunks=[c.content for c in chunks],
                )
                chunks = [
                    _with_updated_content(chunks[i], enriched_contents[i])
                    for i in range(len(chunks))
                ]
                logger.info(
                    "pipeline.contextualized",
                    duration_s=round(time.perf_counter() - context_started, 3),
                    chunk_count=len(chunks),
                    model=self._contextualizer._model,
                    **log_ctx,
                )
...
def _with_updated_content(chunk: PreparedChunk, new_content: str) -> PreparedChunk:
    """Return a copy of ``chunk`` with ``content`` replaced.

    Also recomputes ``chunk_size_tokens`` in metadata and flags the chunk
    as contextualized so downstream consumers (eval, debugging) can tell
    the difference between a raw chunk and one that got situated context.
    """
    from app.utils.token_counter import count_tokens

    meta = chunk.metadata.model_copy(
        update={
            "chunk_size_tokens": count_tokens(new_content),
            # ChunkMetadata has extra="allow" so this extra field is preserved.
            "contextualized": True,
        }
    )
    return PreparedChunk(content=new_content, metadata=meta)
