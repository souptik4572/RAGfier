...
def test_auth_signup_creates_tenant_and_returns_token(monkeypatch) -> None:
    ...
    monkeypatch.setattr(auth_module, "get_service_client", lambda: _FakeClient())
    monkeypatch.setattr(
        auth_module,
        "get_or_create_default_integration",
        lambda client, tid: {"id": integration_id, "tenant_id": tid, "is_default": True},
    )
